...
class SavePacienteActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val lista: List<String> = listOf("CELULAR", "TELEFONE")
        val spinner = findViewById<View>(R.id.et_save_paciente_telefone_tipo) as Spinner
    ...
    fun salvarPaciente(view: View) {
        ...
        var tipoTel = (findViewById<Spinner>(R.id.et_save_paciente_telefone_tipo)).selectedItem.toString()
        var numeroTel =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_numero)).text.toString()

        var rua =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_rua)).text.toString()
        var numeroEnd =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_numero)).text.toString()
        var bairro =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_bairro)).text.toString()
        var estado =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_estado)).text.toString()
        var cep =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_cep)).text.toString()
        var cidade =
                (findViewById<EditText>(R.id.et_save_paciente_telefone_endereco_cidade)).text.toString()
        ...
        RetrofitService().api?.salvarPaciente(cadastroPaciente)?.enqueue(object :
                Callback<CadastrarPacienteDTO?> {
            ...
            override fun onResponse(
                ...
                if (response.code() == 200) {
                    Toast.makeText(
                            this@SavePacienteActivity, "Paciente Cadastrado com Sucesso",
                            Toast.LENGTH_LONG
                    ).show()
                } else {
                    Toast.makeText(
                            this@SavePacienteActivity,
